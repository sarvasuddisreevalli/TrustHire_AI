...
const generateWithRetry = async (model: any, prompt: string, maxRetries = 2) => {
    let retries = 0;
    while (true) {
        try {
...
router.post('/ats-analyze', upload.single('resume'), async (req, res): Promise<any> => {
    try {
...
router.post('/verify-certificate', upload.single('certificate'), async (req, res): Promise<any> => {
    try {
