...
import { apiFetchJobs, apiAnalyzeJob, type JobData } from "@/lib/api";
...
function Jobs() {
  ...
  useEffect(() => {
    Promise.all([apiFetchJobs(), apiFetchApplications()])
      .then(([j, a]) => {
        setJobs(j);
        setAppliedIds(a.map((app: any) => typeof app.jobId === 'string' ? app.jobId : app.jobId._id));
      })
